...
//        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(), Double.NaN)
    val measureValue = healthServicesRepository.measureFlow()
        .onEach { value ->
            viewModelScope.launch {
                if (value is MeasureMessage.MeasureData) {
                    value.hrData.firstOrNull()?.value?.let {
                        healthServicesRepository.sendHrToHandheldDevice(
                            it
                        )
                    }
                }
        ...
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(), MeasureMessage.MeasureData(listOf(), listOf()))

    val stepsValue = passiveDataRepository.latestDailyStep
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(), Double.NaN)

    val recordingEnabled = passiveDataRepository.passiveDataEnabled
    ...
    fun toggleEnabled() {
        viewModelScope.launch {
            val newEnabledStatus = !recordingEnabled.value
